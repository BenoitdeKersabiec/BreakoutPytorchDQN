...
def train(env,
    ...
    :param num_episode: number of episode to run
    ...
    for ep in pbar:
        ...
        for t in itertools.count():
            ...
            if replay_memory.can_sample(batch_size):
                ...
                state_values = estimator(obs_batch / 255.)  # b x VALID ACTIONS
                state_action_values = torch.gather(state_values, 1, act_batch.unsqueeze(1))  # b x 1

                next_state_values_max = target_network(next_obs_batch / 255.).detach().max(dim=1)[0]
                next_state_values = not_done_mask * next_state_values_max

                expected_q_values = (next_state_values * discount) + rew_batch

                loss = loss_func(state_action_values, expected_q_values.unsqueeze(1))
                episode_loss = loss
                ...
                if t == batch_size+2:
                    debug_obs_batch = obs_batch
                    debug_act_batch = act_batch
                    debug_rew_batch = rew_batch
                    debug_done_mask = done_mask
                    debug_next_obs_batch = next_obs_batch
        ...
        pbar.set_description("ep: %d, loss: %.5f, reward: %.2f, time: %d, total_time: %d, exp_size: %d" % (ep,
                                                                                           episode_loss,
                                                                                           episode_reward,
                                                                                           episode_t,
                                                                                           total_t,
                                                                                           replay_memory.num_in_buffer))
    ...
    return debug_obs_batch, debug_act_batch, debug_rew_batch, debug_done_mask, debug_next_obs_batch
